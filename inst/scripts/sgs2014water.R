...
e.sgs.water <- readMSnSet2(file = sgswatertxt, ecol = 1:54)
...
# "BioReplicate" column
  Study_design.sgs.water$BioReplicate <- factor(gsub(".*_(.*)_0.*", "\\1", Study_design.sgs.water$Filename))
...
## feature data
fd <- sgswatertxt$Sequence
fd <- as.data.frame(fd)
colnames(fd) <- "sequence"
fd$Filename <- sgswatertxt$filename
...
fd <- merge(x = fd, y = Study_design.sgs.water, by = "Filename", all.x = TRUE)
fd <- fd[order(fd$sortslot),]
rownames(fd) <- rownames(sgswatertxt)
fd$sortslot <- NULL
